...
export default async function handler(
  ...
  const { method } = req;
  const { id, produto, quantidade, valor } = req.body;

  try {
    switch (method) {
      case 'GET':
        const getAll = await queryDatabase('SELECT * FROM pedidos');
        const formattedGetAll = getAll.rows.map(pedido => ({
          ...pedido,
          data_do_pedido: formatDate(pedido.data_do_pedido),
          valor: `R$${Number(pedido.valor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }).slice(3)}`
        }));
      ...
      case 'POST':
        const postResult = await queryDatabase(
          'INSERT INTO pedidos (produto, quantidade, valor) VALUES ($1, $2, $3) RETURNING *',
